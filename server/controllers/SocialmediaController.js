...
class SocialmediaController
{
        ...
        static addSocial=async (req,res)=>{
            try 
            {
                 ...
                 res.status(200).json({msg:"social media created  successful",data:result});
            }
            catch(err)
            {
                res.status(404).json({error:err.message});  
        ...
        static updateSocial=async (req,res)=>{
            ...
            try {
                ...
                const updatedSocial = await Social.findByIdAndUpdate(id, { $set: update }, { new: true });
                ...
                res.status(201).json({ message: 'Update done successfully', info: updatedSocial });
              } catch (err) {
